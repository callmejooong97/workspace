 ...
 *      ex) int+double → 4byte+8yte =구조체 변수 크기(12byte)
 * 
 * 3. 구조체 변수사용
 ...
 *  - 멤버 사이에 패딩 바이트(padding byte)를 사용해서 정렬 → byte alignment 
 *  - 바이트 얼라인먼트는 멤버 중 크기가 가장 큰 자료형을 기준으로 할당
 *  - 따라서 구조체의 요소와 선언 수서에 따라 필요한 메모리 공간이 다름 (패딩바이트 떄문)
 * 
 * 시험에 무조건나올 것 같다.
 * 
 * ex) int,int,double
 ...
 * 
 *      int,double,int
 *      8byte → int(4byte)+padding(4byte)
 *      8byte → double(8byte)
 *      8byte → int(4byte)+padding(4byte)
 *      ※padding byte 만큼 메모리 낭비
 */
